...
const SelectUser = ({ navigation }) => {
  ...
  const icon = useSelector(state => state.apptheme.icon) || require('../../../assets/images/demoIcon.png');
  ...
  }, [getUsersData, getUsersError, dispatch]);
  ...
  const getData = async () => {
    try {
      ...
      console.log('Error in reading loginData', e);
    }
  };
  ...
  return (
    <LinearGradient colors={['white', 'white']} style={styles.container}>
      <ScrollView showsVerticalScrollIndicator={false} style={{}}>
        <View style={{ height: 140, width: '100%', alignItems: 'center', justifyContent: 'center' }}>
          <Image
            style={{ height: 200, width: 240, resizeMode: 'contain', top: 60 }}
            source={{ uri: icon }}
          />
        ...
        <PoppinsTextMedium style={{ color: 'black', fontSize: 12, marginTop: 20, marginBottom: 10 }} content="Designed and developed by Genefied" />
      </ScrollView>
    </LinearGradient>
  );
};
